...
* 数据传输时发生丢包或延迟的情况，造成 TCP 数据没有按照顺序到达。在 Minecraft 中这些乱序包不会造成影响，因为 TCP 协议会自动重新整理。
  但对于解析器来说，这些乱序包会导致解析错误，我们暂时还没有办法处理。如果你在数据解析错误的前后发现了`TCP Out-Of-Order`或者`TCP Dupe ACK`，那么就是这种情况。
